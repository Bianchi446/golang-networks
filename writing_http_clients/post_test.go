...
func handlePostUser(t *testing.T) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		defer func(r io.ReadCloser) {
			_, _ = io.Copy(io.Discard, r) // `ioutil.Discard` has been deprecated; use `io.Discard`
			_ = r.Close()
		}(r.Body)

		if r.Method != http.MethodPost {
			http.Error(w, "Method not allowed", http.StatusMethodNotAllowed) // Fixed the empty error message
			return
		}
		...
		w.WriteHeader(http.StatusAccepted) // Fixed typo: `WriterHeader` -> `WriteHeader`
	}
}
...
func TestPostUser(t *testing.T) {
	...
	if resp.StatusCode != http.StatusMethodNotAllowed {
		t.Fatalf("Expected status %d; actual staus %d", 
